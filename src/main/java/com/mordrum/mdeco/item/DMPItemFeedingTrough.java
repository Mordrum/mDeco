...
public class DMPItemFeedingTrough extends ItemBlock {
   private DMPBlockFeedingTrough decorationBlock;

   public DMPItemFeedingTrough(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockFeedingTrough)block;
   }
   ...
   public EnumActionResult onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
      ...
      if(posTarget != null && worldIn.isAirBlock(posTarget)) {
         ...
         if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
            IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockFeedingTrough.FACING, enumFacing);
