...
public class DMPItemFireplacePokerSet extends ItemBlock {
   private DMPBlockFireplacePokerSet decorationBlock;

   public DMPItemFireplacePokerSet(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockFireplacePokerSet)block;
   }
      ...
      if(facing == EnumFacing.UP && worldIn.isSideSolid(pos, EnumFacing.UP)) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            ...
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockFireplacePokerSet.FACING, enumFacing);
