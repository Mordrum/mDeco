...
public class DMPItemFireplaceWoodRack extends ItemBlock {
   private DMPBlockFireplaceWoodRack decorationBlock;

   public DMPItemFireplaceWoodRack(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockFireplaceWoodRack)block;
   }
      ...
      if(facing == EnumFacing.UP && worldIn.isSideSolid(pos, EnumFacing.UP)) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            ...
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockFireplaceWoodRack.FACING, enumFacing);
