...
public class DMPItemKitchenPot extends ItemBlock {
   private DMPBlockKitchenPot decorationBlock;

   public DMPItemKitchenPot(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockKitchenPot)block;
   }
      ...
      if(facing == EnumFacing.UP && worldIn.isSideSolid(pos, EnumFacing.UP)) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            ...
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockKitchenPot.FACING, enumFacing);
