...
public class DMPItemKitchenWallUtensils extends ItemBlock {
   private DMPBlockKitchenWallUtensils decorationBlock;

   public DMPItemKitchenWallUtensils(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockKitchenWallUtensils)block;
   }
      ...
      if(facing != EnumFacing.UP && facing != EnumFacing.DOWN) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            ...
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockKitchenWallUtensils.FACING, enumFacing);
