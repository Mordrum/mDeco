...
public class DMPItemFireplaceBellows extends ItemBlock {
   private DMPBlockFireplaceBellows decorationBlock;

   public DMPItemFireplaceBellows(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockFireplaceBellows)block;
   }
      ...
      if(facing == EnumFacing.UP && worldIn.isSideSolid(pos, EnumFacing.UP)) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            ...
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockFireplaceBellows.FACING, enumFacing);
