...
public class DMPItemCoinBag extends ItemBlock {
   private DMPBlockCoinBag decorationBlock;

   public DMPItemCoinBag(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockCoinBag)block;
   }
      ...
      if(facing == EnumFacing.UP && worldIn.isSideSolid(pos, EnumFacing.UP)) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            ...
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockCoinBag.FACING, enumFacing.rotateY());
