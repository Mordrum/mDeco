...
public class DMPItemWallLantern extends ItemBlock {
   private DMPBlockWallLantern decorationBlock;

   public DMPItemWallLantern(Block block) {
      super(block);
      this.setUnlocalizedName(block.getUnlocalizedName().substring(5));
      this.decorationBlock = (DMPBlockWallLantern)block;
   }
      ...
      if(facing != EnumFacing.UP && facing != EnumFacing.DOWN) {
         ...
         if(posTarget != null && worldIn.isAirBlock(posTarget)) {
            if(playerIn.canPlayerEdit(posTarget, facing, stack)) {
               IBlockState iblockstate1 = this.decorationBlock.getDefaultState().withProperty(DMPBlockWallLantern.FACING, facing.getOpposite());
