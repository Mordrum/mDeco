...
public class DMPContainerDeskWoodBasic extends Container {
   private IInventory storageInventory;
   private int numSlots;

   public DMPContainerDeskWoodBasic(World worldIn, BlockPos pos, EntityPlayer player) {
      ...
      int k;
      for(k = 0; k < 4; ++k) {
         this.addSlotToContainer(new Slot(this.storageInventory, slotIndex++, 8 + k * 18, 18));
      }

      for(k = 0; k < 4; ++k) {
         this.addSlotToContainer(new Slot(this.storageInventory, slotIndex++, 8 + k * 18, 36));
      }

      for(k = 0; k < 4; ++k) {
         this.addSlotToContainer(new Slot(this.storageInventory, slotIndex++, 8 + k * 18, 54));
      }

      for(k = 0; k < 4; ++k) {
         this.addSlotToContainer(new Slot(this.storageInventory, slotIndex++, 98 + k * 18, 18));
      }

      for(k = 0; k < 4; ++k) {
         this.addSlotToContainer(new Slot(this.storageInventory, slotIndex++, 98 + k * 18, 36));
      }

      for(k = 0; k < 4; ++k) {
         this.addSlotToContainer(new Slot(this.storageInventory, slotIndex++, 98 + k * 18, 54));
      }
